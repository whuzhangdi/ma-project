function [ temp_mark ] = sortportfolio2( cellwithfactor,fctpos )
%SORTPORTFOLIO sort portfolio over month,last ret,last six ret,last sixty month 
% 1:low 2:middle 3:high 0:no data
%example:[ sortresult ] = sortportfolio2( cellwithfactor,2);
[num_f,~] = size(cellwithfactor);
%% find the maximum length in term of month
max_length = 0;
for i=1:num_f
    temp=cellwithfactor{i,8};
    [~,lie]=size(temp);
    if ~(lie<fctpos)
        temp=temp(:,fctpos);
        [hang,~]=size(temp);
...
for i=1:num_f
    ...
    [~,lie]=size(temp);
    if ~(lie<fctpos)
    temp = cellwithfactor{i,8}(:,fctpos);
    [hang,~] = size(temp);
    if i==26|i==30
        for j=1:hang
